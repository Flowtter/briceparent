...
completePath = [[components, COMPONENTS_PATH]];
...
for (
	var indexComplete = 0;
	indexComplete < completePath.length;
	indexComplete++
) {
	for (var i = 0; i < completePath[indexComplete][0].length; i++) {
		path = pathJoin([
			completePath[indexComplete][1],
			completePath[indexComplete][0][i],
		]);
...
for (var i = 0; i < typeScriptArray.length; i++) {
	typeScriptArray[i] = typeScriptArray[i].substring(
		3,
		typeScriptArray[i].length - 2
	);
}
